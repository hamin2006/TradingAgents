...
| `structured_log.py` | Per-ticker structured JSONL logging of the analyze run: every LLM turn (agent via `langgraph_node` metadata, model, provider, token usage, latency, `finish_reason`), **untruncated** prompts/responses, chain boundaries, tool calls (args), pre-fetch events (`fetch_end`), structured-fallback events, errors; `run_end` event + per-day `summary.json`. Reasoning capture field exists but is not populating (see Pending) |
...
| `watchlist.yaml` | User-facing config: seed watchlist, models, provider pins, capital (10k), `max_tokens: 15000`, sizing, screener + buy-quota (`min_buy_quota: 5`, `max_analyze: 16`) + broker settings, kill switch |
...
- **Reasoning capture not firing** (verified live 2026-09-03: 0 reasoning on all events): OpenRouter returns `message.reasoning` in the SDK message's `model_extra`; LangChain forwards only typed extras and drops it. `_ensure_reasoning_capture` wraps `openai...Completions.create` to stash it thread-locally, but the stash never fires through the framework client path (direct SDK probes show reasoning present) — the actual call path needs tracing. Small fix; defer.
